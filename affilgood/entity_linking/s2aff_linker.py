...
class S2AFFLinker(BaseLinker):
    ...
    def __init__(
        ...
        if data_source != "ror":
            if verbose:
                print(
                    f"S2AFF linker only supports ROR data. Processing ROR data alongside other linkers for '{data_source}'.")
    ...
    def get_single_prediction(self, organization, num_candidates=None):
        ...
        first_stage_candidates, first_stage_scores = self.ror_index.get_candidates_from_main_affiliation(org, location,
                                                                                                         [suborg])
        ...
        if self.debug:
            ...
            print(
                f"Candidates first stage filtered (threshold {THRESHOLD_SCORE_FILTER_FIRSTAGE_EL}, num. candidates:{NUM_CANDIDATES_TO_RERANK}): {candidates_with_scores}")
        ...
        if candidates:
            ...
            if self.rerank and self.pairwise_model:
                ...
                if self.debug:
                    print(
                        f"Retrieving re-ranked candidates for affiliation_string: {affiliation_string}, candidates: {candidates}, scores: {scores}")
                ...
                if self.debug:
                    reranked_candidates_with_scores = [f"{reranked_candidates[i]}:{reranked_scores[i]:.4f}" for i in
                                                       range(len(reranked_candidates))]
