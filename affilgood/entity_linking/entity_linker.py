...
class EntityLinker:
            ...
            for source in self.data_sources:
                ...
                if source in self.linkers and 'Dense' in self.linkers[source]:
                    ...
                    if hasattr(dense_linker, 'hnsw_index') and dense_linker.hnsw_metadata:
                        ...
                        metadata_path = os.path.join(os.path.dirname(dense_linker.hnsw_index_path),
                                                     "org_index_meta.json")
                ...
                if isinstance(linker, str):
                    ...
                    if hasattr(linker, 'data_source'):
                        ...
                        if linker.data_source != data_source:
                            if self.verbose:
                                print(
                                    f"Skipping {linker_name} for {data_source} as it's configured for {linker.data_source}")
                            continue
                    ...
                    if data_source == "wikidata" and hasattr(linker, 'wikidata_countries') and hasattr(linker,
                                                                                                       'wikidata_org_types'):
    ...
    def _should_skip_reranking(self, query_organization, candidates,
        ...
        # If we are returning only the top candidate
            ...
            if len(top_scores) > 1 and top_scores[0] > relative_threshold and (
                    top_scores[0] - top_scores[1]) > gap_threshold:
                return True
                ...
                for mention_idx, (mention, candidates, is_suborg, query_org) in mention_candidates.items():
                    ...
                    if score < adjusted_threshold:
                        if self.verbose:
                            ...
                            top_candidate["explanation"] = best_candidate["explanation"].split("(threshold")[
                                                               0] + f" ({threshold_explanation})"
                            print(
                                f"_format_without_reranking: Candidate {top_candidate.get('name', 'Unknown')} with score {score:.2f} {threshold_explanation}")
                        continue
                    ...
                    if "explanation" in best_candidate:
                        ...
                        best_candidate["explanation"] = best_candidate["explanation"].split("(threshold")[
                                                            0] + f" ({threshold_info})"
                        ...
                        if score >= adjusted_threshold:
                            ...
                            if "explanation" in best_candidate:
                                ...
                                if has_non_latin:
                                    threshold_info += f", adjusted from {self.final_threshold:.2f} for non-Latin text"
                                best_candidate["explanation"] = best_candidate["explanation"].split("(threshold")[
                                                                    0] + f" ({threshold_info}, included as sub-org alternative)"
            ...
            for span_idx, span_candidates in enumerate(item['org_candidates']):
                ...
                for mention_idx, (mention, candidates) in enumerate(span_candidates):
                    ...
                    if skip_reranking:
                        if self.verbose:
                            print(
                                f"_apply_reranking: Skipping reranking for '{mention}': High confidence in retrieval scores")
                        ...
                        try:
                            ...
                            for candidate in reranked_candidates:
                                if "explanation" not in candidate:
                                    ...
                                    candidate[
                                        "explanation"] = f"Reranked from {orig_score:.4f} to {score:.4f} using direct pair matching"
                ...
                for mention_idx, (mention, candidates, is_suborg, query_org) in mention_candidates.items():
                    ...
                    if score < adjusted_threshold:
                        if self.verbose:
                            ...
                            print(
                                f"_apply_reranking: Candidate {top_candidate.get('name', 'Unknown')} with score {score:.2f} {threshold_explanation}")
                        continue
                        ...
                        if score >= adjusted_threshold:
                            ...
                            if "explanation" in best_candidate:
                                ...
                                if "threshold" in best_candidate["explanation"]:
                                    ...
                                    best_candidate[
                                        "explanation"] += f" ({threshold_info}, included as sub-org alternative)"
    ...
    def _format_entity_candidate(self, candidate, score, data_source):
        ...
        if hasattr(self, 'data_manager') and self.data_manager and hasattr(self.data_manager, 'format_id_url'):
            ...
            if data_source == 'ror':
                ...
                id_url = f"https://www.wikidata.org/wiki/{entity_id}" if not entity_id.startswith(
                    'https://') else entity_id
            else:
                id_url = entity_id
        ...
        for result in results:
            ...
            for span_candidates in result["org_candidates"]:
                ...
                for mention, candidates in span_candidates:
                    ...
                    # If the number of candidates included in the output should be reduced to show_num_candidates.
                    top_candidates = []
