...
class WhooshLinker(BaseLinker):
    ...
    def __init__(
            ...
            # Whether to add WikiData labels from previously downloaded file when generating the ROR indices
    ):
        """
    ...
    def initialize(self):
        ...
        if self.run_score_calibration:
            ...
            if stats["perfect_matches"]["count"] > 0:
                ...
                if self.debug:
                    print(
                        f"Calibrated normalization: max={self.normalization_max_score:.2f}, threshold={self.normalization_min_threshold:.2f}")
    ...
    def _normalize_score(self, raw_score, org_name=None):
        ...
        if raw_score >= thresholds["perfect"]:
            ...
            return scores["near_perfect"]["min"] + position * (
                        scores["near_perfect"]["max"] - scores["near_perfect"]["min"])
    ...
    def _clean_organization_name(self, name):
        ...
        filtered_words = [word for word in words if
                          word.lower() not in self.stopwords and word not in self.legal_entities]
                ...
                if i < perfect_count:
                    ...
                    if is_non_latin:
                        ...
                        if random.random() > 0.5:
                            near_perfect_name = " ".join(
                                [w for i, w in enumerate(words) if i != random.randint(0, len(words) - 1)])
                        else:
        ...
        for hit in results:
            ...
            if best_match_score > 0:
                ...
                if self.debug:
                    ...
                    print(
                        f"=> Name candidate: {name} - best match for {search_text}: {match_type} {best_match_type}: '{best_match_text}' - Score boosted by {boost_factor:.2f}x. Updated hit score: {hit.score}")
    ...
    def get_candidate_matches(self, organization):
        ...
        clean_name_variants = {self._clean_organization_name(name_variant) for name_variant in name_variants if
                               name_variant.strip()}
