...
class NER:
        ...
        for item in ner_results:
            ...
            for span_idx, (span, span_ner, span_ner_raw) in enumerate(zip(span_entities, ner, ner_raw)):
                ...
                for i in range(len(all_entities) - 1):
                    ...
                    is_next_org = next_entity['entity_group'] == MAINORG_NER_LABEL or next_entity[
                        'entity_group'] == SUBORG_NER_LABEL
    ...
    def recognize_entities(self, text_list, batch_size=None):
        ...
        results = [{"raw_text": item["raw_text"], "span_entities": item["span_entities"], "ner": [], "ner_raw": []} for
                   item in text_list]
    ...
    def change_suborg(self, entities):
        ...
        for entity in entities:
            if entity["entity_group"] == SUB_NER_LABEL and any(
                    [word.startswith(pref) for word in entity["word"].lower().split() for pref in SUB_TO_SUBORG]):
