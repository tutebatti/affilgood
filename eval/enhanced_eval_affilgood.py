...
def main():
    ...
    parser.add_argument("--skip-translation", action="store_true", help="Skip language_prediction/translation")
    parser.add_argument("--external-llm-translation", action="store_true", help="Use external LLM for translation")
    parser.add_argument('--linker-threshold', type=float, default=INITIAL_THRESHOLD_ENTITY_LINKING,
                        help='Threshold score for entity linking.')
    parser.add_argument('--final-threshold', type=float, default=FINAL_THRESHOLD_ENTITY_LINKING,
                        help='Final threshold score.')
    ...
    parser.add_argument('--use-cache-retrieval', action="store_true",
                        help='Enable caching for entity linking retrieval')
    parser.add_argument('--use-cache-reranking', action="store_true",
                        help='Enable caching for entity linking reranking')
    ...
    if rerank and hasattr(entity_linking_handler, 'reranker') and entity_linking_handler.reranker:
        ...
        if args.use_cache_reranking and hasattr(entity_linking_handler.reranker, 'set_cache_enabled'):
            ...
            if hasattr(entity_linking_handler.reranker, 'set_cache_enabled'):
                ...
                if args.verbose:
                    print(
                        f"Disabled cache for reranker (direct attribute): {entity_linking_handler.reranker.__class__.__name__}")
    ...
    config_params["component_params"]["ner"] = {
        "model_path": affilgood.ner.span_model if hasattr(affilgood.ner, 'model_path') else 'N/A',
    ...
    config_params["component_params"]["language_preprocessing"] = {
        ...
        "model": getattr(affilgood.language_preprocessor, 'model_name', 'N/A') if hasattr(affilgood,
                                                                                          'language_preprocessor') else 'N/A'
    }
